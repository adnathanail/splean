...
Rendering lives with the representation it draws, in `Axiomatic/Render.lean` (`zxEquivHtml?` for a `≈z` goal, `zxDiagramHtml?` for a bare term); `Panel.lean` keeps only the panel, the presenter and `#zx`. `ZXPanel` and `zxPresenter` are `ZXDiagram`-only, and both renderers evaluate the `Expr` to a real `ZXDiagram` via `evalZXDiagram` (`Meta.evalExpr`), so they need closed terms.

Undrawable is not an error, but the renderers do not decide that: they return `none` only for "not this kind of expression at all" and *throw* on a term of the right shape they cannot draw. `softly` in `Panel.lean` is the single place that turns that into a `none` (letting interrupts and recursion-depth failures through, since neither is an answer about the diagram). So on a goal with free variables or an unassigned metavariable LHS, `ZXPanel` shows "No ZX diagram" and `zxPresenter` throws — which is how `getExprPresentations` knows to omit it. It is also why `#zx` softens its `ZXDiagram` attempt: an open one falls through to the error naming the type, rather than surfacing `evalExpr`'s complaint. A presenter that cannot render **must** throw rather than return empty HTML.
